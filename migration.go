...
import (
	...
	"github.com/incu6us/goose/v3/internal/sqlparser"
)
...
func (m *Migration) run(ctx context.Context, db *sql.DB, direction bool) error {
	...
	case ".go":
		if !m.Registered {
			return fmt.Errorf("ERROR %v: failed to run Go migration: Go functions must be registered and built into a custom binary (see https://github.com/incu6us/goose/tree/master/examples/go-migrations)", m.Source)
		}
		start := time.Now()
		var empty bool
		if m.UseTx {
